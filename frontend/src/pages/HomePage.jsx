    ...
    <div className="min-h-screen bg-black">
      ...
      {notificationBar && notificationBar.is_active && notificationBar.text && (
        <div 
          ...
          style={{ backgroundColor: notificationBar.bg_color, color: notificationBar.text_color }}
        >
          {notificationBar.link ? (
            <a href={notificationBar.link} className="hover:underline">{notificationBar.text}</a>
          ) : (
      ...
      <section className="pt-16 lg:pt-20" data-testid="reviews-section">
        ...
        <div className="py-6 lg:py-8 bg-gradient-to-b from-black to-transparent overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="font-heading text-2xl sm:text-3xl md:text-4xl font-bold text-white uppercase tracking-tight mb-4 lg:mb-6 text-center animate-fade-in">
        ...
        <section className="py-8 lg:py-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-heading text-xl sm:text-2xl font-bold text-white uppercase tracking-tight flex items-center gap-2">
                <span className="text-2xl">🔥</span> Hot Deals
              </h2>
              <Link to="/" className="text-gold-500 text-sm hover:underline flex items-center gap-1">
      ...
      {bestSellers.length > 0 && (
        <section className="py-8 lg:py-10 bg-card/30">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-heading text-xl sm:text-2xl font-bold text-white uppercase tracking-tight flex items-center gap-2">
                <span className="text-2xl">⭐</span> Best Sellers
              </h2>
              <Link to="/" className="text-gold-500 text-sm hover:underline flex items-center gap-1">
                View All <ChevronRight className="h-4 w-4" />
              </Link>
      ...
      <section ref={productsSectionRef} className="py-8 lg:py-12" data-testid="products-section">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-6 lg:mb-8">
            <h2 className="font-heading text-3xl sm:text-4xl md:text-5xl font-bold text-white uppercase tracking-tight mb-2 lg:mb-3 animate-fade-in">
              All <span className="text-gold-500">Products</span>
            </h2>
          ...
          {searchQuery && (
            <div className="flex items-center justify-center gap-2 mb-4 lg:mb-6 animate-scale-in">
              <div className="flex items-center gap-2 bg-gold-500/10 border border-gold-500/30 rounded-full px-4 py-2">
                ...
                <button onClick={clearSearch} className="ml-1 p-1 hover:bg-white/10 rounded-full transition-colors">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 mb-6 lg:mb-8">
            <button
              ...
              className={`category-pill px-3 lg:px-4 py-1.5 rounded-full border text-xs lg:text-sm font-heading uppercase tracking-wider transition-all duration-300 hover:scale-105 ${
      ...
      {paymentMethods.length > 0 && (
        <section className="py-8 lg:py-10 bg-gradient-to-r from-gold-500/10 via-gold-500/5 to-gold-500/10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="font-heading text-xl sm:text-2xl font-bold text-white uppercase tracking-tight mb-4 text-center">
              Payment Options
            </h2>
      ...
      <section className="py-8 lg:py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-heading text-2xl sm:text-3xl font-bold text-white uppercase tracking-tight mb-6 text-center">
            Trust & <span className="text-gold-500">Safety</span>
          </h2>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-heading text-2xl sm:text-3xl font-bold text-white uppercase tracking-tight">
                Guides & <span className="text-gold-500">Tips</span>
              </h2>
              <Link to="/blog" className="text-gold-500 text-sm hover:underline flex items-center gap-1">
                View All <ChevronRight className="h-4 w-4" />
              </Link>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {blogPosts.map((post) => (
                <Link
                  key={post.id}
                  to={`/blog/${post.slug}`}
                  className="bg-card border border-white/10 rounded-lg overflow-hidden hover:border-gold-500/50 transition-all duration-300 group"
                >
                  {post.image_url && (
                    <img src={post.image_url} alt={post.title} className="w-full h-32 object-cover" />
                  )}
                  <div className="p-4">
                    <h3 className="font-heading font-semibold text-white group-hover:text-gold-500 transition-colors line-clamp-2">
                      {post.title}
                    </h3>
